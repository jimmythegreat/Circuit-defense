...
- **2026-06-11 · v1.8.4** — Follow-up to the same report (owner: *"the combo meter still overlaps the milestone [pop] in the top left corner"*). v1.8.3 had moved the `🔥 N× COMBO!` milestone burst/floater into the **top-left** corner to clear the centered bonus text — but that's where the persistent COMBO meter lives, so it overlapped the meter. The pop now fires on the **center board, below the top HUD band** (`W/2,114` / `W/2,132`), measured clear of the meter (65px below), the round-clear bonus (59px below), and the boss HP bar. Render-only; Test 11 layout assertion updated; suite 114/0 green.
