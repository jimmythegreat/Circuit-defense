...
- **Version / What's New**: `GAME_VERSION` constant (semver-ish, bumped every routine run) + `CHANGELOG_ENTRIES` array (last ~10, newest first, mirrors CHANGELOG.md) near the top of the script. `openWhatsNew()`/`closeWhatsNew()` toggle the `#whatsnew` **side panel** (an `<aside>` that is a sibling of `#gameWrap` inside the `#stage` flex row — NOT a modal overlay). It sits to the right of the canvas on wide viewports, wraps below on narrow ones (`flex-wrap`), is height-capped to the canvas (`max-height:562px`) and scrolls internally via `#wnList`. The start screen has a `✨ What's New` button and a clickable `#verTag` showing the version (set in `renderStartScreen`); the panel has a `✕` (`#wnClose`) to hide it. When you ship a run, bump `GAME_VERSION` and prepend a `CHANGELOG_ENTRIES` entry.
