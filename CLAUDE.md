...
- **Version / What's New**: `GAME_VERSION` constant (semver-ish, bumped every routine run) + `CHANGELOG_ENTRIES` array (last ~10, newest first, mirrors CHANGELOG.md) near the top of the script. The `#whatsnew` **side panel** (an `<aside>` that is a sibling of `#gameWrap` inside the `#stage` flex row — NOT a modal overlay) is **open by default** and sits *flush* against the canvas's right edge (`#stage` gap 0, `border-radius:0 8px 8px 0`); it wraps below on narrow viewports (`flex-wrap`), is height-capped to the canvas (`max-height:562px`) and scrolls internally via `#wnList`. `initWhatsNew()` (called at startup) opens it unless the player closed it before — `closeWhatsNew()` persists `cd_wnclosed='1'`, `openWhatsNew()` clears it. The start screen has a `✨ What's New` button + clickable `#verTag` (version, set in `renderStartScreen`) to reopen, and the panel has a `✕` (`#wnClose`) to hide it. When you ship a run, bump `GAME_VERSION` and prepend a `CHANGELOG_ENTRIES` entry.
