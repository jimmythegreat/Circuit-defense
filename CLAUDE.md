...
- **Version / What's New**: `GAME_VERSION` constant (semver-ish, bumped every routine run) + `CHANGELOG_ENTRIES` array (last ~10, newest first, mirrors CHANGELOG.md) near the top of the script. `openWhatsNew()`/`closeWhatsNew()` drive the `#whatsnew` modal; the start screen has a `✨ What's New` button and a clickable `#verTag` showing the version (set in `renderStartScreen`). When you ship a run, bump `GAME_VERSION` and prepend a `CHANGELOG_ENTRIES` entry.
