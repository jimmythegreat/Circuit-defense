...
- **Achievements** (v1.5.0): `ACHIEVEMENTS` array (8 badges) + `ACH_BY_ID`. `meta.achievements` is an id→true map; `meta.stats = {dmg, runs}` are lifetime counters. `grantAchievements(won)` is called once per finished run from `endGame(false)`/`winGame(true)` — it tallies `stats`, evaluates each badge's condition, and returns the newly-unlocked list, which `achLine()` appends to the overlay text. Per-run `livesLostThisRun` flag (set wherever a life leaks; forced `true` in `loadRun()` so resumed runs can't earn **Flawless**). Start-screen 🏅 button → `openAchievements()`/`renderAchievements()` panel (`#achPanel`, mirrors talents); `#achBtn` shows `done/total` (updated in `renderStartScreen`). Adding a badge = push to `ACHIEVEMENTS` + a condition line in `grantAchievements`.
...
- localStorage keys all prefixed `cd_`: `cd_save`, `cd_meta`, `cd_campaign`, `cd_best_<diff>`, `cd_mute`, `cd_wnclosed` (What's New panel hidden by player)
