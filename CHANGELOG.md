...
- **🔩 Overengineered — a new rare perk (a fresh "tall build" damage axis):** each tower deals **+4% damage per upgrade level above 1** — L1 → +0%, L5 → +16%, L7 (with 🌟 Overdrive) → +24%. It rewards sinking gold into a few high-level towers rather than spreading it thin, the strategic **inverse of** the wide-build 🏛️ Phalanx (per tower count) and 🌈 Overwhelm (per distinct type) — the three form a non-dominated tall/wide/diverse trio. Wired in `effDmg` via `(t.level-1)`; `upgradeKey()` already hashes `t.level`, so the panel steps up on each upgrade (not every frame). "Too easy"-safe: conditional on a real gold investment and bounded by the max-level cap — below the flat 💎 Diamond Core (+30%). `overengineered` lives in `perkState` (default false). A rare, so the legendary-only Wildcard skips it. Test group `[227]`.
- **🏋️ Heavy Ordnance — a new legendary perk (a fresh rate/damage trade-off):** **+60% damage per shot, but −25% fire rate.** Net DPS is only ~+20% (1.6 × 0.75), so it's not power creep (below Diamond Core's flat +30%) — it favours slow **burst** towers (Sniper/Cannon/Mortar/Railgun, whose big per-shot hits punch through flat armor) while blunting rapid/swarm towers (Gun/Tesla/Pulsar lose uptime). The rate/damage **inverse of ⏱️ Hair Trigger** (which trades damage for rate). Wired in `effDmg` (×1.6) + `effRate` (×1.3333 = slower reload); both hashed by `upgradeKey()`. `heavyOrd` lives in `perkState` (default false). A legendary — `resolveWildcard()` can roll it. Test group `[228]`.
- **💰 Bounty — a new (11th) targeting mode:** a tower set to Bounty aims at whichever in-range enemy is worth the **most gold**, popping valuable escorts (warden 2.4× / breacher 2.5× / bastion 2.2× / herald 2.2×), tanks and bosses first to bank income + feed the combo meter. An **economy/value axis** distinct from 'strong' (a low-HP high-bounty warden outranks a full-HP plain tank) and from 'boss'; it trades leak-safety (ignores the leader) for income, degrading to 'first' when bounties tie. Auto-wired via `MODES`/`MODE_ICON`; `pickTarget()` `case 'bounty'` in cd-combat.js; `loadRun` validates it. Test group `[229]`.
- **⚙️ Peak Performance — a new achievement (roster 56 → 57):** win with **6+ towers at max level** — the deeper rung above 🏗️ Maxed Out (3+), rewarding a heavily-invested core (pairs with 🔩 Overengineered's per-level damage). Reads the final board like Maxed Out; win-gated. Additive/save-safe. Test group `[230]`.
