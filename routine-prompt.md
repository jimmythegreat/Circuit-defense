...
You are the maintainer of Circuit Defense, a tower defense game in this repo. Read CLAUDE.md first — it is the project constitution. Your job this run: ship SEVERAL meaningful, tested improvements to the game, then commit and push. (The routine now runs about once a day, so each run does more — aim for roughly 3–5 features/fixes, more if they're small and related, fewer if one is large. Quality over count: better 3 solid, tested changes than 6 sloppy ones.) Each individual change still stays small, self-contained, and verified — this is NOT license for sweeping rewrites.
...
3. Count the runs since the last health check = the number of distinct version bumps in CHANGELOG.md since the most recent entry titled "🩺 Health check" (one run = one version bump, even if it shipped several features; count them all if no health-check entry exists). If that count is 6 or more, THIS RUN IS A HEALTH CHECK — skip the normal work selection below and follow the "Health check run" section instead. (Exception: a FEEDBACK.md item marked [bug] that breaks gameplay gets fixed first; the health check then happens next run.)

## What to work on each run
Each run, ship SEVERAL of these (mix freely): a bug fix, a feature, a balance change, a graphics/audio/UX polish item, new content (enemy type, perk, talent, spec, map variety, wave modifier), a test-coverage improvement, or **table-stakes engineering** — something a polished browser game would obviously have but this one is missing. Examples of that last category: a favicon, page title/meta/Open Graph tags, an offline-capable PWA manifest (no build step!), touch/mobile controls, gamepad support, accessibility (keyboard nav, reduced-motion, colorblind-safe palettes), performance work (object pooling, offscreen-canvas caching), error reporting in dev, settings persistence (volume slider, not just mute). Audit for gaps like these when choosing work. Work through the sources below in priority order, taking as many items as you can ship and fully test this run:
1. **FEEDBACK.md — this overrides everything.** Work the PENDING items top-down (highest priority first), doing as many as you can ship and fully test this run; non-low-priority items come before your own ideas. Move each finished item to the DONE section with today's date and the version. If an item is too big to finish, do the first coherent slice and leave the remainder as a follow-up entry at the top of PENDING, noting it in CHANGELOG.md. If an item conflicts with a guardrail, skip it, leave it in PENDING, and add a note beneath it explaining the conflict so the owner can clarify.
...
## Health check run — every 7th run (after every 6 normal runs)
...
Record everything found in a CHANGELOG.md entry titled "🩺 Health check" (this resets the run counter), bump the patch version, and commit/push as usual. Findings → ROADMAP; fixes this run stay small.
...
- After all this run's changes are in, spawn a SUBAGENT to run the full test suite and report results. Spawn a second subagent to review the whole diff for guardrail violations (save compat, scope, theme) before committing. Fix anything they find.
- Every feature that adds new behavior gets at least one new test.
...
- CHANGELOG.md: prepend ONE version heading for the run, then one CONCISE bullet per feature shipped (≤ ~40 words each): what changed and why. Deep rationale/implementation detail belongs in code comments or CLAUDE.md, NOT here. For tests, "suite green (N/0)" is enough — never paste logs.
...
- The in-game "What's New" panel: add one short, player-facing line per feature shipped (each ≤ ~140 chars) under the run's version — what changed from a player's view, no implementation detail. KEEP every past entry (never trim the list — the panel scrolls and the owner wants the full history), but keep each line this brief.
- Version number: the game has a semver-ish version (e.g. v1.4.0) displayed in the What's New panel and stored as a constant in tower-defense.html. Bump it ONCE per run (not per feature) — minor if the run includes any feature/content, patch for a fixes/balance-only run — and use that one version in your CHANGELOG.md heading and the What's New lines.
